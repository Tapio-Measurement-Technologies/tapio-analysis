...
class AnalysisController(AnalysisControllerBase, FrequencyMarksMixin, ExportMixin):
    ...
    def plot(self):
        ...
        # The top axis carries the Hz reading only where the Hz reading is shown
        # at all; otherwise it is the wavelength, which every window can give.
    ...
    def mark_amplitude_at(self, freq):
        return self.get_spectrum_amplitude_at(freq)
...
class AnalysisWindow(AnalysisWindowBase[AnalysisController], AnalysisRangeMixin, ChannelMixin, FrequencyRangeMixin, MachineSpeedMixin,
    ...
    def select_frequency_at(self, ax, xdata):
        """Select the spectrum bin nearest to a position on the frequency axis."""
        ...
        if not (xlim[0] <= xdata <= xlim[1]) or xdata < 0:
            return False  # Do not proceed if the x-coordinate is out of bounds
