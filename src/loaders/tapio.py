...
open_file_types = "All Files (*);;Calibration files (*.ca2);;Data files (*.da2);;Header files (*.pk2);;Paper machine files (*.pmdata.json);;CD Sample location files (*.samples.json)"
...
def load_data(main_window):
    ...
    fileNames, _ = QFileDialog.getOpenFileNames(
        ...
        options=options)
    if fileNames:
        if len(main_window.windows) > 0:
            response = QMessageBox.question(main_window, 'Confirm open new file',
                                            'This will close all current analysis windows. Do you want to proceed?',
                                            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No, QMessageBox.StandardButton.No)
            if response == QMessageBox.StandardButton.No:
                return

        main_window.closeAll()
