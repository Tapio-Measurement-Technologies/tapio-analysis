...
from gui.components import AnalysisRangeMixin, DoubleChannelMixin, FrequencyRangeMixin, MachineSpeedMixin, SampleSelectMixin, SpectrumLengthMixin, ShowWavelengthMixin, CopyPlotMixin, AutoDetectPeaksMixin, ChildWindowCloseMixin
...
from controllers import CoherenceController
import settings


class CoherenceWindow(QWidget, DataMixin, AnalysisRangeMixin, DoubleChannelMixin, FrequencyRangeMixin, MachineSpeedMixin,
                      SampleSelectMixin, SpectrumLengthMixin, ShowWavelengthMixin, CopyPlotMixin, AutoDetectPeaksMixin,
                      ChildWindowCloseMixin):

    def __init__(self, window_type="MD", controller: CoherenceController | None = None):
        super().__init__()
        self.dataMixin = DataMixin.getInstance()
        self.window_type = window_type
        self.controller = controller if controller else CoherenceController(
            window_type)
    ...
    def initUI(self):
        ...
        self.setWindowTitle(f"{self.window_type} Coherence analysis ({
