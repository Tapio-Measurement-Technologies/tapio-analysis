...
class BandPassFilterMixin:
    ...
    def addBandPassRangeSlider(self, layout, live_update=settings.UPDATE_ON_SLIDE):
        ...
        self.bandPassFilterSlider.setDecimals(settings.BAND_PASS_FILTER_DECIMALS)
    ...
    def loadExtraData(self):
        try:
            ...
            if file_path:
                ...
                self.extraDataAdjustStartSlider.setMinimum(-1 * settings.EXTRA_DATA_ADJUST_RANGE)
                self.extraDataAdjustStartSlider.setMaximum(settings.EXTRA_DATA_ADJUST_RANGE)
                self.extraDataAdjustStartSlider.setValue(0)
                self.extraDataAdjustStartSlider.show()

                self.extraDataAdjustEndSlider.setMinimum(-1 * settings.EXTRA_DATA_ADJUST_RANGE)
                self.extraDataAdjustEndSlider.setMaximum(settings.EXTRA_DATA_ADJUST_RANGE)
